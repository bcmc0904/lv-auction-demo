...
export default function AuctionDetails({params}: {params: {id: string}}) {
  ...
  const renderActionHistories = (histories: Array<any>) => {
    ...
    return tmp.map((item: any, index: number) => <BidTeaser auctionId={data._id} order={index + 1} {...item} key={index}/>)
  }
    ...
    <div className="auction-item" key={update}>
      <div className="flex gap-2 flex-wrap auction-info">
        ...
        <div className="actions box flex flex-col gap-4">
          ...
          <div className="flex justify-between items-center bid-action">
            <div className="input-wrapper">
              <div className="input">
                <button
                  disabled={!validDate() || loading}
                  onClick={() => {toggleBidPrice(false)}}
                >-</button>
                <span className="value">{bidPrice ? numberFormat(bidPrice) : 0}</span>
                <button
                  disabled={!validDate() || loading}
                  onClick={() => {toggleBidPrice(true)}}
                >+</button>
              </div>
              <div>
                <button
                ...
                >Đặt giá cao hơn mức giá cao nhất 01 (một) bước giá</button>
              </div>
            </div>
            <div className="btns">
              <button
